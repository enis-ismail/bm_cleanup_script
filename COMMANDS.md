...
Remove preferences marked for deletion by `analyze-preferences`. Uses **per-realm deletion files** so each realm only deletes preferences that actually exist on it.

**Required config:**
- At least one realm in `config.json`
- Per-realm deletion files generated by `analyze-preferences` (e.g., `results/development/EU05/EU05_preferences_for_deletion.txt`)
...
STEP 3: Select Deletion Level
  → Choose tier (cascading):
     ...
     6) Realm-targeted — All tiers, respects realm tags (legacy)

STEP 4: Load Per-Realm Deletion Lists
  → Loads {realm}_preferences_for_deletion.txt for each selected realm
...
STEP 5: Review Per-Realm Deletion Files
  → Opens each realm's deletion file in VS Code for manual review
...
STEP 6: Create Backups (Per Realm)
  ...
  → Uses per-realm deletion file for each realm's backup
...
STEP 8: Remove Preferences
  → Calls OCAPI DELETE for each preference per realm
  → Each realm only deletes preferences from its own file
...
| Realm-targeted | All tiers | Legacy — all tiers, respects per-realm tags |
