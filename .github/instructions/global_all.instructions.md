...
| `bm_*` | BM | M | Business Manager | gmvp
...
Golden Rule (Nonâ€‘Negotiable): The file global_all.instructions.md may ONLY be created (or recreated) by executing the script _generate_global_instructions.sh; if it already exists before a successful script run, it is INVALID and must be deleted before proceeding. Manual creation, editing, patching, reflowing, summarizing, truncating, compressing, or regenerating its content by any other means (including AI tool direct writes) is strictly forbidden. The script must: (1) emit a threeâ€‘line frontmatter block exactly (---, applyTo: "**", ---), then a blank line, then the byteâ€‘exact contents of AGENT_DEV_SETUP.md; (2) compute SHA256 of the source file and of the generated body (excluding the first 4 lines/frontmatter) and require they MATCH; (3) verify line counts (source vs. generated body) are IDENTICAL; (4) append or verify a provenance marker (e.g. a final comment line beginning with # Provenance: containing the original hash and UTC timestamp); (5) abort with nonâ€‘zero exit on ANY mismatch (hash, line count, missing provenance, prior manual file, or empty body) and NOT attempt autoâ€‘repair. No subsequent steps (including generating AGENTS.md or cartridge instruction files) may begin until a successful run prints both a success hash line and a â€œsecondary verification passedâ€ message. Any downstream artifact produced while this rule is violated is void and must be discarded after remediation.
...
if [ "$original_hash" != "$body_hash" ]; then
  echo "âŒ ERROR: Hash mismatch detected" >&2
...
echo "âœ… Global instructions file created with verified integrity"
...
if [ "$orig_lines" != "$global_body_lines" ]; then
  echo "âŒ ERROR: Line count mismatch (source=$orig_lines, body=$global_body_lines)" >&2
  exit 1
fi
...
Expected output:
âœ… Global instructions file created with verified integrity
